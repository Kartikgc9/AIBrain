...
const similarity_1 = require("../utils/similarity"); // Ensure this is available or use store's search
class IngestionPipeline {
    constructor(store, llm) {
        this.store = store;
        this.llm = llm;
        this.extractor = new ExtractionService_1.ExtractionService(llm);
    }
    async run(text, source, userId = 'local_user') {
        ...
        for (const candidate of candidates) {
            ...
            if (similarMemories.length > 0) {
                ...
                const similarity = (0, similarity_1.cosineSimilarity)(embedding, topMatch.embedding);
                if (similarity > 0.95) {
